...
def load_zh_catalog(zh_tsv: Path) -> dict[str, str]:
    """Read data/cwe.zh.tsv into a {cwe_id: name_zh} map.

    Missing file OR empty Name_Zh column → no entry (the renderer
    then falls back to the English name from the slim catalog, per
    issue #2's "if not, use English" requirement).

    The Chinese catalog is generated by .x-cmd/cwe_zh.py from
    MITRE's official Chinese mirror at cwe.org.cn; it covers ~90%
    of CWE entries. The rest (drafts, views, deprecated ids without
    a CN translation yet) gracefully fall back to English.
    """
    out: dict[str, str] = {}
    if not zh_tsv.is_file():
        return out
    with zh_tsv.open("r", encoding="utf-8", newline="") as fh:
        next(fh, None)   # skip header
        for line in fh:
            cols = line.rstrip("\n").split("\t")
            if len(cols) < 2:
...
def aggregate(
    ...
    When `zh` is provided AND has an entry for the id, that Chinese
    name is used as `name`; otherwise we fall back to the English
    name from the slim catalog. The same `name` field is what the
    markdown renderer prints — i.e. the TSV + markdown language
    choice is made here, not downstream.
    """
    rows: list[tuple[str, str, int, float, float]] = []
    for cid, name_en in catalog:
        name = (zh or {}).get(cid) or name_en
...
def write_report(out: Path,
    ...
    The `name` column is whatever the caller put in (English by
    default, or Chinese when `zh` was applied via `aggregate`). The
    machine-readable TSV stays in whichever language the renderer
    wanted — we don't emit two TSVs, because the slim catalog
    (English) is already published as a release asset.
    """
    if rank_by == "count":
...
def write_report_md(out: Path,
                    rows_since: list[tuple[str, str, int, float, float]],
                    *,
                    zh: bool = False) -> None:
    ...
    `zh=True` swaps the headline / question / subtitle prose to
    Chinese while leaving the table headers in English-Chinese mix
    (CJK column labels render fine on github.com). The CWE `Name`
    column is whatever the caller put in the row tuples — we don't
    translate inside this function; the `zh` is applied earlier via
    `aggregate(rows, ..., zh=zh_map)`.
    """
    out.parent.mkdir(parents=True, exist_ok=True)
    ...
    with out.open("w", encoding="utf-8") as fh:
        if zh:
            fh.write(f"自 {SINCE_YEAR} 年以来，共 {since_cve:,} 个 CVE，"
                     f"涉及 {since_cwe:,} 个不同的 CWE。\n\n")
            fh.write(f"### {SINCE_YEAR} 年以来，工程师们最常犯的错误是什么？\n\n")
            fh.write(f"_Top {TOP_N} CWE by CVE count —— 按 CVE 数量排序的前 {TOP_N}。_\n\n")
        else:
        ...
        if zh:
            fh.write(f"\n### {SINCE_YEAR} 年以来，犯下这些错误后果有多严重？\n\n")
            fh.write(f"_Top {TOP_N} CWE by average CVSS score —— 至少 "
                     f"{MIN_CVE_FOR_SCORE_RANK} 个 CVE 以避免单个 CWE 极端值的影响。_\n\n")
        else:
...
def main(argv: list[str]) -> int:
    ...
    # Chinese catalog (optional). If data/cwe.zh.tsv is missing or
    # empty we silently fall back to the English slim catalog — see
    # issue #2: "如果没有，就用英文" (if no Chinese, use English).
    zh = load_zh_catalog(data_dir / "cwe.zh.tsv")
    if zh:
        cov = f"{len(zh)}/{len(catalog)}" if catalog else f"{len(zh)}"
        print(f"loaded {cov} CWE Chinese names from data/cwe.zh.tsv",
              file=sys.stderr)
    else:
        print("warn: data/cwe.zh.tsv missing or empty — Chinese README "
              "will fall back to English names. Run .x-cmd/cwe_zh.py "
    ...
    # Since-SINCE_YEAR pool — drives the two since-TSVs + the MD.
    ...
    # Same pool again, but with Chinese names substituted where the
    # zh catalog has them. Used ONLY for the markdown — the TSVs
    # stay English so x cwe consumers see a stable, language-
    # independent identifier (English names are the canonical form
    # upstream of MITRE).
    rows_since_zh = aggregate(catalog, scores_by_cwe_since, zh=zh)

    # Four TSV outputs, two ranking axes × two time windows. These
    # stay English so the release asset's hash is stable regardless
    # of whether the Chinese catalog has been refreshed.
    outputs = [
    ...
    #   cwe.report.md     — English (for README.md)
    #   cwe.report.zh.md  — Chinese (for README.cn.md, see issue #2)
    # The Chinese version uses Chinese names wherever the zh catalog
    # has them and falls back to English names otherwise (handled
    # inside aggregate via zh=...); it does NOT emit when zh is
    # empty so a no-op fallback doesn't create an empty file.
    write_report_md(report_dir / "cwe.report.md", rows_since)
    print(f"wrote {report_dir / 'cwe.report.md'}", file=sys.stderr)
    if zh:
        write_report_md(report_dir / "cwe.report.zh.md", rows_since_zh, zh=True)
        print(f"wrote {report_dir / 'cwe.report.zh.md'}", file=sys.stderr)
